...
class jvm_sys(system_interface):
    ...
    cgroups = None
    period = 100000
    keys = ["think", "e1_bl", "e1_ex","t1_sw","t1_hw","e2_bl", "e2_ex","t2_sw","t2_hw"]
    isCpu = None
    tier_socket = None
    ...
    def startClient(self, pop):
        ...
        subprocess.Popen([javaCmd, "-Xmx30G", "-Xms30G",
                         ...
                         '--queues', '[\"think\", \"e1_bl\", \"e1_ex\", \"t1_sw\" ,\"t1_hw\",\"e2_bl\", \"e2_ex\", \"t2_sw\" ,\"t2_hw\"]'],stdout=f, stderr=f1)
        f.close()
        f1.close()
        self.waitClient()
